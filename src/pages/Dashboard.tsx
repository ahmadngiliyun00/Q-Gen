...
const fetchExternalTitle = async (
  url: string,
  id: string,
  accessToken?: string,
): Promise<{ name: string | null; isPublic?: boolean; mimeType?: string }> => {
  if (id && accessToken) {
    try {
      const res = await fetch(
        `https://www.googleapis.com/drive/v3/files/${id}?fields=name,permissions,mimeType`,
        {
      ...
      if (res.ok) {
        ...
        if (data.permissions) {
          isPublic = data.permissions.some(
            (p: any) => p.type === "anyone" || p.id === "anyoneWithLink",
          );
        } else {
          isPublic = false;
        }
        return { name: data.name || null, isPublic, mimeType: data.mimeType };
      }
    } catch (err) {
      console.warn("Drive API metadata fetch failed, falling back", err);
    }
  }
    ...
    if (res.ok) {
      ...
      if (match && match[1]) {
        let name = match[1]
          ...
          .replace(/&quot;/gi, '"');
        name = name.replace(
          /\s*-\s*Google\s+(Docs|Drive|Sheets|Slides|Forms)$/i,
          "",
        );
        if (!name.includes("Google Accounts") && !name.includes("Sign in")) {
          return { name: name.trim() };
        }
      }
    }
  } catch (err) {
    console.error("Failed to fetch title via allorigins", err);
  }
...
export default function Dashboard() {
  ...
  const [renamingItem, setRenamingItem] = useState<QuizHistoryItem | null>(
    null,
  );
  ...
  useEffect(() => {
    try {
      ...
      if (savedMateri) {
        const parsedSavedLinks = JSON.parse(savedMateri);
        setSavedLinks(parsedSavedLinks);

        // Background fetch titles that are still URLs
        parsedSavedLinks.forEach((link: LinkItem) => {
          if (link.name === link.url || link.name.startsWith("http")) {
            const match =
              link.url.match(/id=([a-zA-Z0-9_-]+)/) ||
              link.url.match(/\/d\/([a-zA-Z0-9_-]+)/) ||
              link.url.match(/\/folders\/([a-zA-Z0-9_-]+)/);
            const id = match ? match[1] : "";
            fetchExternalTitle(link.url, id, user?.accessToken).then((data) => {
              if (data.name && data.name !== link.url) {
                setSavedLinks((prev) => {
                  const updated = prev.map((l) =>
                    l.id === link.id
                      ? {
                          ...
                          isPublic: data.isPublic ?? l.isPublic,
                          mimeType: data.mimeType || l.mimeType,
                        }
                      : l,
                  );
                  localStorage.setItem(
                    `qgen_saved_materi_${email}`,
      ...
      const savedLinksCurrent = localStorage.getItem(
        `qgen_current_links_${email}`,
      );
      if (savedLinksCurrent) {
        const parsedLinks = JSON.parse(savedLinksCurrent);
      ...
      const savedInstructions = localStorage.getItem(
        `qgen_additional_instructions_${email}`,
      );
  ...
  const [previewIsPublic, setPreviewIsPublic] = useState<boolean | undefined>(
    undefined,
  );
  ...
  React.useEffect(() => {
    ...
    // Check if it's a valid URL, if not, skip
    try {
      new URL(url);
    } catch (e) {
      setPreviewTitle(null);
    ...
    fetchExternalTitle(url, id, user?.accessToken)
      .then((data) => {
        if (data.name) setPreviewTitle(data.name);
        else setPreviewTitle(null);
        if (data.mimeType) setPreviewMimeType(data.mimeType);
        else setPreviewMimeType(null);
        setPreviewIsPublic(data.isPublic);
      })
      .catch(() => {
        setPreviewTitle(null);
        setPreviewIsPublic(undefined);
  ...
  useEffect(() => {
    localStorage.setItem(
      `qgen_current_links_${user?.email || "guest"}`,
  ...
  useEffect(() => {
    localStorage.setItem(
      `qgen_selected_links_${user?.email || "guest"}`,
      JSON.stringify(Array.from(selectedLinks)),
    );
  }, [selectedLinks, user?.email]);

  useEffect(() => {
    localStorage.setItem(
      `qgen_additional_instructions_${user?.email || "guest"}`,
  ...
  useEffect(() => {
    localStorage.setItem(
      `qgen_question_count_${user?.email || "guest"}`,
      JSON.stringify(questionCount),
    );
  ...
  const handleSaveToMateri = () => {
    ...
    if (url && !savedLinks.find((l) => l.url === url)) {
      ...
      localStorage.setItem(
        `qgen_saved_materi_${user?.email || "guest"}`,
        JSON.stringify(newSaved),
      );
      addToast("Tersimpan", "Materi berhasil disimpan ke sidebar", "success");

      if (!previewTitle) {
        fetchExternalTitle(url, id, user?.accessToken).then((data) => {
          if (data.name) {
            setSavedLinks((prev) => {
              const updated = prev.map((l) =>
                l.id === newSavedId
    ...
    if (url && !links.find((l) => l.url === url)) {
      ...
      setLinks((prev) => [
        ...prev,
        {
          ...
          name,
          isPublic,
          mimeType: previewMimeType || undefined,
        },
      ]);
  ...
  const handleGenerate = async () => {
    ...
    import("../lib/gemini")
      .then(async ({ generateQuizStream }) => {
        let isError = false;
        let checkComplete = false;
        let accumulatedText = "";

        const activeLinkTitles = links
        ...
        const finalResult = await generateQuizStream(
          ...
          (chunk) => {
            ...
            if (!checkComplete) {
              ...
              if (
                upperText.includes("ERROR_INSUFFICIENT_DATA") ||
                upperText.includes("KESALAHAN SAAT MEMBUAT SOAL")
              ) {
                isError = true;
                checkComplete = true;
              ...
              const upperAccumulated = accumulatedText
                .trimStart()
                .toUpperCase();
              if (
                upperAccumulated.includes("ERROR_INSUFFICIENT_DATA") ||
                upperAccumulated.includes("KESALAHAN SAAT MEMBUAT SOAL")
              ) {
                isError = true;
              }
            }
        ...
        if (
          finalUpper.includes("ERROR_INSUFFICIENT_DATA") ||
          finalUpper.includes("KESALAHAN SAAT MEMBUAT SOAL")
        ) {
          isError = true;
        }
        ...
        if (isError) {
          ...
          addToast(
            ...
            finalResult
              .replace(/\*?\*?ERROR_INSUFFICIENT_DATA:?\*?\*?\s*/i, "")
              .trim() ||
              "Input materi tidak memadai untuk dibuatkan soal atau terjadi kesalahan server.",
            "error",
          ...
          try {
            const saved = localStorage.getItem(
              `qgen_history_${user?.email || "guest"}`,
            );
            ...
            const rawExcerpt =
              additionalInstructions ||
              activeLinkTitles.join(", ") ||
              finalResult.substring(0, 60);
            const excerpt =
              rawExcerpt.substring(0, 70) +
              (rawExcerpt.length > 70 ? "..." : "");
  ...
  const handleRenameSubmit = (e: React.FormEvent) => {
    ...
    if (renamingItem && renameInput.trim()) {
      const newHistory = history.map((h) =>
        h.id === renamingItem.id ? { ...h, title: renameInput.trim() } : h,
      );
    ...
    if (file) {
      ...
      reader.onload = (event) => {
        try {
          ...
          if (
            data &&
            (Array.isArray(data.history) || Array.isArray(data.savedLinks))
          ) {
            const newHistory = Array.isArray(data.history)
              ? data.history.slice(0, isGuest ? 3 : 25)
              : [];
            const newSavedLinks = Array.isArray(data.savedLinks)
              ? data.savedLinks.slice(0, isGuest ? 5 : 25)
              : [];
    ...
    return (
      <Editor
        ...
        onSaveToDrive={() => {
          if (activeItem) {
            const newHistory = history.map((h) =>
              h.id === activeItem.id ? { ...h, isSavedToDrive: true } : h,
            );
            setHistory(newHistory);
            localStorage.setItem(
              `qgen_history_${user?.email || "guest"}`,
        ...
        onSaveContent={(newContent) => {
          if (activeItem) {
            const newHistory = history.map((h) =>
              h.id === activeItem.id ? { ...h, content: newContent } : h,
            );
            setHistory(newHistory);
            localStorage.setItem(
              `qgen_history_${user?.email || "guest"}`,
      ...
      <header className="h-16 bg-primary text-white flex items-center justify-between px-8 shadow-md z-20 shrink-0">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-xl font-semibold tracking-tight">
            Q-Gen{" "}
            <span className="hidden md:inline font-light text-blue-100">
        ...
        <div className="flex items-center gap-4">
          ...
          <button
            ...
            title={
              theme === "dark" ? "Matikan Mode Gelap" : "Aktifkan Mode Gelap"
            }
          >
            {theme === "dark" ? (
              ...
              {history.length === 0 ? (
                <div className="text-center py-8 px-4 border border-dashed border-border rounded-xl bg-background shadow-sm">
                  ...
                  <p className="text-xs text-text-muted mt-1.5 leading-relaxed">
                    Penilaian yang Anda hasilkan akan muncul di sini.
                  </p>
                </div>
              ) : (
                <div className="space-y-2 max-h-[250px] overflow-y-auto pr-1">
                  {history.map((item) => (
                    <div
                      key={item.id}
                      className="relative group w-full flex items-center bg-surface border border-border rounded-lg shadow-sm hover:border-primary hover:shadow transition-all overflow-hidden p-2"
                    >
                      <button
                        onClick={() => {
                      ...
                      >
                        <div className="flex items-start justify-between">
                          <h3
                            className="text-[13px] font-bold text-text-primary mb-1 truncate"
                            title={
                              item.title ||
                              `${item.questionCount} Soal ${item.format}`
                            }
                          >
                            {item.title ||
                              `${item.questionCount} Soal ${item.format}`}
                        ...
                        {item.excerpt && (
                          <p className="text-[11px] text-text-muted line-clamp-2 leading-tight mb-1">
                            {item.excerpt}
                          </p>
                        )}
                        <p className="text-[9px] text-text-tertiary font-mono font-medium">
                      ...
                      <div className="shrink-0 flex items-center gap-1 pl-1">
                        <button
                          ...
                          className="p-2 text-text-muted hover:text-red-500 hover:bg-red-500/10 rounded border border-transparent hover:border-red-500/20 transition-colors"
                        >
                          <Trash2 className="w-4 h-4" />
                        </button>
                        <button
                          ...
                          title="Ubah Nama"
                          className="p-2 text-text-muted hover:text-primary hover:bg-primary/10 rounded border border-transparent hover:border-primary/20 transition-colors"
                        >
                          <Edit3 className="w-4 h-4" />
                        </button>
                      </div>
              ...
              {savedLinks.length === 0 ? (
                <div className="text-center py-6 px-4 border border-dashed border-border rounded-xl bg-background shadow-sm">
                  <p className="text-[11px] text-text-muted leading-relaxed">
                    ...
                    <br />
                    Simpan materi melalui input utama.
                  </p>
                </div>
              ) : (
                <div className="space-y-2 max-h-[250px] overflow-y-auto pr-1">
                  {savedLinks.map((item) => (
                    ...
                    >
                      <div className="flex-1 min-w-0 pl-1 pr-3">
                        ...
                        <p
                          className="text-[10px] text-text-muted truncate opacity-80 font-mono"
                          title={item.url}
                        >
                          {item.url.match(/id=([a-zA-Z0-9_-]+)/)?.[1] ||
                            item.url.match(/\/d\/([a-zA-Z0-9_-]+)/)?.[1] ||
                            item.url.match(
                              /\/folders\/([a-zA-Z0-9_-]+)/,
                            )?.[1] ||
                            item.url}
                      ...
                      <div className="flex shrink-0 items-center justify-end gap-1.5 pl-1 border-l border-border/50">
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            const newSaved = savedLinks.filter(
                              (l) => l.id !== item.id,
                            );
                            setSavedLinks(newSaved);
                            localStorage.setItem(
                          ...
                          onClick={(e) => {
                            ...
                            if (!links.find((l) => l.url === item.url)) {
                              if (links.length >= 10) {
                                addToast(
                                  "Penuh",
                                  "Maksimal 10 tautan dapat ditambah",
                                  "error",
                                );
                              } else {
                                const newLinkId = crypto.randomUUID();
          ...
          <div className="mt-auto border-t border-border pt-6 space-y-4">
            ...
            <div className="p-4 bg-background rounded-lg border border-border">
              ...
              <p className="text-[10px] text-text-muted leading-relaxed">
                Terhubung ke Gemini 3.1 Pro. <br />
                {isGuest ? "Maksimal 5 soal per sesi." : "Fitur Pro aktif."}
              </p>
            </div>
            ...
            <div className="flex items-center justify-between bg-background pl-3 rounded-xl border border-border shadow-sm overflow-hidden mt-4">
              <div className="flex items-center gap-3 py-3">
                ...
                <span className="text-sm font-semibold text-text-primary">
                  {user?.name || (isGuest ? "Pengguna Tamu" : "Pendidik")}
                </span>
              </div>
              <button
                onClick={logout}
        ...
        <main className="flex-1 overflow-y-auto p-4 lg:p-8 w-full">
          <div className="max-w-4xl w-full mx-auto space-y-6 pb-4">
            <header className="space-y-2">
              <h1 className="text-4xl font-extrabold tracking-tight text-text-primary drop-shadow-sm">
                Buat Paket Soal Baru
              </h1>
              <p className="text-text-muted font-medium text-lg">
                Sediakan materi pembelajaran Anda dan atur format kuis yang
                diinginkan.
              </p>
            </header>
            ...
            <section className="bg-surface rounded-2xl shadow-lg border border-border p-5 sm:p-7 space-y-6 transition-all duration-300 hover:shadow-xl hover:-translate-y-1">
                ...
                <div className="flex items-center gap-2 sm:gap-3">
                  {links.length > 0 && selectedLinks.size > 0 && (
                    <button
                      onClick={() => {
                        setLinks((prev) =>
                          prev.filter((l) => !selectedLinks.has(l.id)),
                        );
                        setSelectedLinks(new Set());
                      }}
              ...
              <form onSubmit={handleAddLink} className="relative">
                <div className="flex shadow-sm rounded-lg border border-border focus-within:ring-4 focus-within:ring-primary/20 focus-within:border-primary transition-all overflow-hidden bg-background">
                  ...
                  {currentLink.length >= 15 && isValidUrl && (
                    <button
                      type="button"
                  ...
                  <button
                    id="add-link-btn"
                    type="submit"
                    disabled={!currentLink.trim() || !isValidUrl}
                  ...
                  >
                    ...
                    <kbd className="hidden md:inline-block ml-1 px-1.5 py-0.5 text-[10px] uppercase font-mono bg-white/20 rounded text-white border border-white/30">
                      Enter
                    </kbd>
                  </button>
                </div>
                {!isValidUrl && (
                  <div className="absolute -bottom-6 left-1 text-xs text-red-500 font-medium">
                    Tautan tidak valid. Harus diawali dengan http:// atau
                    https://
                  </div>
                )}
              </form>
                  ...
                  {links.map((link) => {
                    ...
                    return (
                      <motion.div
                        ...
                        key={link.id}
                        className={cn(
                          "flex items-center gap-2 px-3 py-1.5 rounded-full text-sm font-medium shadow-sm transition-all hover:shadow hover:-translate-y-0.5 border",
                          fileColorStyle,
                        )}
                      >
                        <input
                        ...
                        {link.isPublic !== undefined && (
                          ...
                          >
                            ...
                            <span>
                              {link.isPublic ? "Public" : "Read-Only"}
                            </span>
                          </div>
                        )}
              ...
              {links.length === 0 && (
                <div className="text-center p-8 border-2 border-dashed border-border rounded-xl text-text-muted text-sm">
                  Tempel tautan di atas atau ketik instruksi di bawah untuk
                  memulai. Penguraian folder rekursif didukung.
                </div>
              )}
            </section>
              ...
              <div className="bg-surface rounded-2xl shadow-lg border border-border p-5 sm:p-7 space-y-4 transition-all duration-300 hover:shadow-xl hover:-translate-y-1">
                ...
                <div className="text-xs text-blue-50 bg-gradient-to-r from-blue-900 via-blue-800 to-blue-700 border border-blue-600/50 p-4 md:p-5 rounded-xl flex items-start gap-3 md:gap-4 shadow-[0_4px_14px_0_rgba(29,78,216,0.39)]">
                  ...
                  <p className="leading-relaxed font-medium">
                    ...
                    Berikan topik atau paragraf materi konkret jika tidak
                    melampirkan berkas Drive.
                  </p>
                </div>
              </div>
            </section>
              ...
              <button
                ...
                className={cn(
                  ...
                  !isGenerateClicked &&
                    !isGenerating &&
                    selectedLinks.size > 0 &&
                    "animate-button-glow hover:animate-none active:animate-none",
                )}
              >
                {isGenerating ? (
                  <>
          ...
          <footer className="mt-8 py-6 text-center text-text-muted text-sm border-t border-border/50 max-w-4xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4">
            <p>
              &copy; 2026 Q-Gen | Instant Exams, Zero Burnout. All rights
              reserved.
            </p>
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between p-3 bg-background rounded-xl border border-border">
                  ...
                  <div className="flex items-center gap-1.5">
                    <kbd className="px-2 py-1 bg-surface border border-border rounded text-xs font-mono font-bold text-text-primary shadow-sm">
                      {isMac ? "Cmd ⌘" : "Ctrl"}
                    </kbd>
